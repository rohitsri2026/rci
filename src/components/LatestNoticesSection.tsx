...
export default function LatestNoticesSection() {
  ...
  return (
    <section aria-labelledby="latest-notices-title" className="py-8 sm:py-12 bg-white border-b border-slate-200/80 relative overflow-hidden">
      <div className="container mx-auto px-3.5 sm:px-6 lg:px-8 max-w-7xl">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2.5 mb-5 sm:mb-7">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-100/80 border border-blue-200 text-blue-700 text-[10px] sm:text-xs font-black uppercase tracking-wider">
              <Megaphone className="w-3.5 h-3.5 text-blue-600" />
              <span>Official RCI Notice Board</span>
            </div>
            <h2 id="latest-notices-title" className="text-2xl sm:text-3xl font-black font-display text-[#07152F] tracking-tight leading-tight">
          ...
          <Link
            href="/notices"
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-[#07152F] border border-slate-300 text-xs font-bold transition-all shadow-2xs hover:shadow-xs shrink-0 self-start sm:self-auto min-h-[38px]"
          >
            <span>View All Notices</span>
            <ChevronRight className="w-3.5 h-3.5 text-[#155EEF]" />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-5">
          {notices.slice(0, 3).map((notice) => (
            <div
              key={notice.id}
              className={`bg-white rounded-xl sm:rounded-2xl p-3.5 sm:p-5 border shadow-2xs space-y-2.5 flex flex-col justify-between transition-all hover:shadow-sm ${
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between gap-2">
                  {notice.priority === "urgent" ? (
                    ...
                    <span className="px-2 py-0.5 rounded-md bg-[#D4A72C] text-slate-950 text-[9px] font-black uppercase tracking-wider">
                      IMPORTANT
                    </span>
                  ) : (
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                {notice.button_url && notice.button_text ? (
                  <Link
                    ...
                    className="inline-flex items-center gap-1 text-xs font-bold text-[#155EEF] hover:text-blue-700 hover:underline min-h-[36px]"
                  >
                  ...
                  <Link
                    href="/notices"
                    className="inline-flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-slate-900 min-h-[36px]"
                  >
