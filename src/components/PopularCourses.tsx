...
export default function PopularCourses() {
  ...
  return (
    <section aria-labelledby="popular-courses-title" className="py-10 sm:py-14 bg-slate-50 border-b border-slate-200/80">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6 sm:mb-8">
          <div className="max-w-xl">
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1 font-normal">
              Practical diploma and accounting courses designed for immediate employment.
            </p>
          </div>

          <div className="flex items-center justify-between sm:justify-end gap-3">
            {/* Mobile swipe helper text */}
            <span className="sm:hidden text-[11px] font-bold text-blue-600 bg-blue-50/80 px-2.5 py-1 rounded-full border border-blue-100">
              ← Swipe courses →
            </span>

            <Link
              href="/courses"
              className="hidden sm:inline-flex items-center gap-1.5 px-4 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-900 font-bold text-xs sm:text-sm transition-all shadow-2xs hover:shadow-sm hover:border-blue-300 shrink-0"
            >
        ...
        {/* 3 Featured Courses: Swipe carousel on mobile, 3-column grid on desktop */}
        <div className="flex sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-6 overflow-x-auto sm:overflow-visible pb-3 sm:pb-0 snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
          {displayCourses.slice(0, 3).map((course, idx) => {
            ...
            return (
              <div
                key={course.id || idx}
                className="w-[82vw] max-w-[310px] sm:w-auto sm:max-w-none snap-center bg-white rounded-2xl border border-slate-200/90 shadow-2xs hover:shadow-md hover:border-blue-300 transition-all duration-200 overflow-hidden flex flex-col justify-between group shrink-0 sm:shrink"
              >
                <div>
                  {/* Thumbnail */}
                  <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100">
                    <Image
                      ...
                      fill
                      sizes="(max-width: 640px) 310px, (max-width: 1024px) 50vw, 33vw"
                    ...
                    <div className="absolute top-2.5 left-2.5 z-10">
                      <span className="bg-white/95 backdrop-blur-xs text-[#07152F] text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md shadow-2xs">
                        {category}
                      </span>
                    </div>

                    {/* ISO Badge */}
                    <div className="absolute bottom-2 right-2 flex items-center gap-1 text-white text-[9.5px] font-bold bg-black/60 backdrop-blur-xs px-2 py-0.5 rounded z-10 border border-white/15">
                  ...
                  <div className="p-3.5 sm:p-5">
                    ...
                    <p className="text-xs text-slate-500 mb-3 leading-relaxed line-clamp-2 min-h-[2rem]">
                ...
                <div className="px-3.5 sm:px-5 pb-3.5 sm:pb-5 pt-0">
                  <div className="grid grid-cols-2 gap-2">
                    <Link
                      href={`/courses/${slug}`}
                      className="flex items-center justify-center gap-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-bold transition-colors min-h-[42px]"
                    >
                    ...
                    <Link
                      ...
                      className="flex items-center justify-center gap-1 py-2 bg-[#155EEF] hover:bg-blue-700 text-white rounded-xl text-xs font-extrabold transition-colors shadow-2xs min-h-[42px]"
                    >
                      <span>Apply Now</span>
        ...
        <div className="mt-5 text-center sm:hidden">
          <Link
            href="/courses"
            className="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-slate-300 bg-white active:bg-slate-50 text-[#07152F] font-bold text-xs shadow-2xs min-h-[44px]"
          >
            <span>View All Courses</span>
            <ArrowRight className="w-3.5 h-3.5 text-[#155EEF]" />
          </Link>
