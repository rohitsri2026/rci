-- ====================================================================
-- RCI WEBSITE CENTRALIZED CMS DATABASE MIGRATION & SETUP SCRIPT
-- Run this script in your Supabase SQL Editor
-- ====================================================================

-- 1. SITE SETTINGS TABLE (Branding & Core Site Info)
...
CREATE TABLE IF NOT EXISTS public.navigation_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  location VARCHAR(50) NOT NULL, -- 'header', 'footer_quick', 'footer_useful'
  label VARCHAR(100) NOT NULL,
  url VARCHAR(255) NOT NULL,
  open_new_tab BOOLEAN NOT NULL DEFAULT false,
  display_order INT NOT NULL DEFAULT 0,
  is_active BOOLEAN NOT NULL DEFAULT true,
  is_system BOOLEAN NOT NULL DEFAULT false, -- Protect critical system routes
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);
...
-- ====================================================================
-- SUPABASE STORAGE BUCKET CREATION FOR WEBSITE ASSETS
-- ====================================================================
...
-- Macro function to simplify adding public SELECT and authenticated ALL policies
DO $$
DECLARE
  tbl text;
  tables text[] := ARRAY[
