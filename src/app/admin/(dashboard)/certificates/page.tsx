    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          ...
          className="flex items-center gap-2 bg-green-600 text-white px-5 py-3 rounded-xl font-semibold hover:bg-green-700 transition-colors text-sm"
        >
              ...
              {certs && certs.length > 0 ? (
                certs.map((cert: any) => (
                  <tr key={cert.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-slate-900">{cert.students?.name}</td>
                    <td className="px-6 py-4 text-slate-600">{cert.courses?.name}</td>
